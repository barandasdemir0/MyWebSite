...
public class BlogPostManager :GenericManager<BlogPost,BlogPostDto,CreateBlogPostDto,UpdateBlogPostDto>, IBlogPostService
{
    ...
    public override async Task<BlogPostDto> AddAsync(CreateBlogPostDto dto, CancellationToken cancellationToken = default)
    {
    ...
    public override async Task<List<BlogPostDto>> GetAllAsync(CancellationToken cancellationToken = default)
    {
    ...
    public override async Task<BlogPostDto?> GetByIdAsync(Guid guid, CancellationToken cancellationToken = default)
    {
        var entity = await _blogPostDal.GetByIdAsync(guid, tracking: false, includes: source => source.Include(x => x.BlogTopics).ThenInclude(y => y.Topic), cancellationToken: cancellationToken);
        if (entity == null)
        {
            return null;
        }
    ...
    {
        var entity = await _blogPostDal.GetAsync(x => x.Slug == slug,
            tracking: false,
            includes: source => source.Include(x => x.BlogTopics).ThenInclude(y => y.Topic), cancellationToken: cancellationToken);
        if (entity == null)
        {
