// ============================================
// PORTFOLIO PAGE - Category Filtering
// ============================================

document.addEventListener('DOMContentLoaded', function () {
    const portfolioPage = document.querySelector('.portfolio-section');
    ...
    // Add CSS transitions to portfolio cards
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            portfolioCards.forEach(card => {
                const title = card.querySelector('.portfolio-info h4').textContent.toLowerCase();
            ...
            if (!hasResults && query.length > 0) {
                if (!noResultsMsg) {
                    ...
                    document.querySelector('.portfolio-grid').appendChild(noResultsMsg);
                }
...
// Pagination functionality for portfolio page
document.addEventListener('DOMContentLoaded', function() {
    const paginationBtns = document.querySelectorAll('.pagination-btn');
    
    if (paginationBtns.length === 0) return; // Not on portfolio page
    ...
    paginationBtns.forEach((btn) => {
        btn.addEventListener('click', function() {
            ...
            if (isNumber) {
                ...
                console.log('Portfolio - Sayfa:', pageNumber);
            ...
            updatePortfolioPaginationArrows();
        });
    });
    
    function updatePortfolioPaginationArrows() {
        const currentActive = document.querySelector('.pagination-btn.active');
        const currentPage = parseInt(currentActive?.textContent || 1);
